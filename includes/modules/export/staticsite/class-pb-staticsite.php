...
class StaticSite extends Export {
	...
	function convert() {
		...
		while ( $post_id = current( $pos ) ) {
			if ($prevPostID != -1) {
				$postHelper[$prevPostID]["next"]=$post_id;
			}
		...
		reset( $pos );
		$metadata['postHelper'] = $postHelper;
		/****** DONE CREATING postHelper DATA STRUCTURE *********/
	...
	protected function createContainer() {
		mkdir( $this->tmpDir . '/siteFiles' );
		mkdir( $this->tmpDir . '/siteFiles/assets' );
	}


	/**
	 * Create siteFiles/* files.
	 *
	...
	protected function createStylesheet($metadata) {
		...
		$path_to_tmp_stylesheet = $this->tmpDir . "/siteFiles/{$this->stylesheet}";
	...
	protected function scrapeKneadAndSaveCss( $path_to_original_stylesheet, $path_to_copy_of_stylesheet, $metadata ) {
		...
		$path_to_epub_assets = $this->tmpDir . '/siteFiles/assets';
	...
	protected function createCover( $book_contents, $metadata ) {
		...
		$dest_path = $this->tmpDir . "/siteFiles/assets/" . $dest_image;
	...
	/**
	 * Parse HTML snippet, download all found <img> tags into /siteFiles/assets/, return the HTML with changed <img> paths.
	 *
